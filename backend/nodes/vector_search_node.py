...
class VectorSearchNode(BaseNode):
    ...
    def config_fields(self) -> list[NodeConfigField]:
        return [
            ...
            NodeConfigField(
                ...
                field_type="state_variable",
                required=False,
                help_text='State variable containing a filters JSON string, typically generated by an upstream LLM node. Example: {"department =": "cardiology", "year >=": 2020}. Supported operators: =, !=, <, <=, >, >=, LIKE, NOT LIKE, IS, IS NOT.',
            ),
            NodeConfigField(
    ...
    def execute(self, state: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
        ...
        if filters_from:
            raw_filters = state.get(filters_from, "")
            if raw_filters:
